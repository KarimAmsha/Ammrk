...
class SweetsViewController: UIViewController, CLLocationManagerDelegate {
...
extension SweetsViewController {
...
//        self.dwSelectRestaurantType.placeholder = "Hall Type".localize_
//        self.dwSelectRestaurantType.font = UIFont(name: "NeoSansArabic", size: 18)
    ...
    func setupData() {
        ...
        self.halasType = UserProfile.shared.constantsData?.halaCats ?? [:]
...
extension SweetsViewController {
    ...
    func getData() {
        ...
        if !( self.cityId != nil || self.isReview != nil || self.lat != nil || self.lng != nil) {
            self.btnClearFilter.isHidden = true
        } else {
        ...
        let request = BaseRequest()
        request.url = GlobalConstants.HALAS
        request.method = .get
...
extension SweetsViewController: UITableViewDataSource, UITableViewDelegate {
...
extension SweetsViewController: UITextFieldDelegate {
...
extension SweetsViewController: FSPagerViewDelegate, FSPagerViewDataSource {
...
extension SweetsViewController: DZNEmptyDataSetSource, DZNEmptyDataSetDelegate {
