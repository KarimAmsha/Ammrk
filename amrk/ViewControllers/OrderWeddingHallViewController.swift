/*********************        Yousef El-Madhoun        *********************/
//
//  OrderWeddingHallViewController.swift
//  amrk
//
...
class OrderWeddingHallViewController: UIViewController {
    ...
    @IBOutlet weak var chKosha: BEMCheckBox!
    
    @IBOutlet weak var lblKosha: UILabel!
    
    @IBOutlet weak var chDisplayScreen: BEMCheckBox!
    
    @IBOutlet weak var lblDisplayScreen: UILabel!
    
    @IBOutlet weak var chPhoto: BEMCheckBox!
    ...
    @IBOutlet weak var lblHallPolicy: UILabel!
    ...
    guard let dis = self.txtCode.text, dis != "" else {
...
//        datePicker?.show()
    ...
    @IBAction func btnKosha(_ sender: Any) {
        if self.chKosha.on {
            self.chKosha.on = false
            self.lblKosha.textColor = .opaqueSeparator
        } else {
            self.chKosha.on = true
            self.lblKosha.textColor = "#219CD8".color_
        }
    }
    
    @IBAction func btnScreen(_ sender: Any) {
        if self.chDisplayScreen.on {
            self.chDisplayScreen.on = false
            self.lblDisplayScreen.textColor = .opaqueSeparator
        } else {
            self.chDisplayScreen.on = true
            self.lblDisplayScreen.textColor = "#219CD8".color_
        }
    }
    ...
    @IBAction func btnPhoto(_ sender: Any) {
        if self.chPhoto.on {
            self.chPhoto.on = false
            self.lblPhoto.textColor = .opaqueSeparator
        } else {
            self.chPhoto.on = true
            self.lblPhoto.textColor = "#219CD8".color_
        }
    }
...
//        }
    ...
    func submitDataToApplePay() {
        ...
        request.parameters = [
            ...
            "owner_email":  "mail",
            "mobile":  "",
            "discount_code": self.txtCode.text ?? "",
            "kosha": self.chKosha.on ? 1 : 0,
            ...
            "screens": self.chDisplayScreen.on ? 1 : 0,
            "video_and_photo": self.chPhoto.on ? 1 : 0
        ]
...
extension OrderWeddingHallViewController {
    
    func setupView() {
        self.lblTitle.text = self.object?.name
        self.imgHall.imageURL(url: self.object?.image ?? "")
        self.lblHallPolicy.text = self.object?.userInfo?.policy
        
        self.txtPhoneNumber.delegate = self
        
        self.dwPersonCount.placeholder = "Select Wedding".localize_
    ...
    func setupData() {
//        for value in stride(from: 50, to: 550, by: 50){
...
//        }
        ...
        if MOLHLanguage.currentAppleLanguage() == "ar" {
            str = "Type".localize_ + "\t\t" + "Wedding capacity".localize_ + "\t" + "Price person".localize_
        } else {
            str = "Type".localize_ + "\t\t" + "Wedding capacity".localize_ + "\t\t" + "Price person".localize_
        }
...
//                self.showSnackbarMessage(message: "We hope to pay in order to confirm your order".localize_, isError: false)
                ...
                if self.paymentMethodType == .visa {
...
extension OrderWeddingHallViewController: UITextFieldDelegate {
...
extension OrderWeddingHallViewController: UITextViewDelegate {
...
extension OrderWeddingHallViewController: PKPaymentAuthorizationViewControllerDelegate {
