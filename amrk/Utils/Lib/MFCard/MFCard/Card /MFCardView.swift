...
@IBDesignable public class MFCardView: UIView {
    ...
    fileprivate func generalSetup(){
        ...
        btnDone.setTitle(" Close ".localize_, for: .normal)
    ...
    internal func fillTextFieldFromCard(card:Card){
        ...
        if (card.number?.count)!<6 && toast{
            UIApplication.topViewController()?.view.makeToast("Wrong card number".localize_)
        }else{
    ...
    fileprivate func setPlaceholder(_ allow:Bool){
        if allow{
            txtCvc.placeholder      = "###"
            txtCardName.placeholder = "Enter Name".localize_
    ...
    @IBAction func btnDoneAction(_ sender: AnyObject) {
        if (btnDone.title(for: .normal) == " Close ".localize_){
            dismissCard()
            ...
            if cardNumber.count <= 13 {
                error = "Please enter valid card number".localize_
            }else if viewExpiryMonth.labelValue.text! == "MM".localize_ {
                error = "Please Select Expiry Month".localize_
            }else if viewExpiryYear.labelValue.text! == "YYYY".localize_ {
                error = "Please Select Expiry Year".localize_
            }else if (txtCvc.text?.count)! < 3 {
                error = "Please enter valid Cvv".localize_
            }
    ...
    fileprivate func hideDoneButton(){
        UIView.animate(withDuration: 1, animations: {
            self.btnDone.setTitle(" Close ".localize_, for: .normal)
        })
    }
    ...
    @objc fileprivate func flip(_ sender:UISwipeGestureRecognizer?) {
        ...
        if (cardFrontView.isHidden == false) {
            ...
            btnCvc.setTitle("CARD".localize_, for: UIControl.State())
            ...
            btnCvc.setTitle("CVC".localize_, for: UIControl.State())
            UIView.transition(with: cardBackView, duration: 1.0, options: transitionOptions, animations: {
